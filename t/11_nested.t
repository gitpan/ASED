...
sub test_maxclient {
    ...
    my @lines =  split /\n/, $diff;
    if ( $lines[0] ne "129,130c129,130" ) {
	print "not ok $testno\n";
    } elsif ( $lines[1] ne "< #ErrorLog logs/host.foo.com-error_log" ) {
	print "not ok $testno\n";
    } elsif ( $lines[2] ne "< #TransferLog logs/host.foo.com-access_log" ) {
	print "not ok $testno\n";
    } elsif ( $lines[4] ne "> #ErrorLog logs/MYCOMPANY.com-error_log" ) {
	print "not ok $testno\n";
    } elsif ( $lines[5] ne "> #TransferLog logs/MYCOMPANY.com-access_log" ) {
	print "not ok $testno\n";
    } else {
	print "ok $testno\n";
    }
